...
# 计算皮尔逊相关系数
corr_C_R, p_C_R = pearsonr(C_normalized, R_normalized)
corr_N_R, p_N_R = pearsonr(N_normalized, R_normalized)
corr_B_R, p_B_R = pearsonr(B_normalized, R_normalized)

# 创建 DataFrame
data = {
    'Variable': ['C_prime', 'N_prime', 'B_prime'],
    'Pearson Correlation Coefficient': [corr_C_R, corr_N_R, corr_B_R],
    'p-value': [p_C_R, p_N_R, p_B_R]
}
...
print(df.to_string(formatters={
    'Pearson Correlation Coefficient': '{:.4f}'.format,
    'p-value': '{:.4f}'.format
}))


# 打印相关性结果
print(f"Pearson correlation between C' and R': {corr_C_R:.4f}")
print(f"Pearson correlation between N' and R': {corr_N_R:.4f}")
print(f"Pearson correlation between B' and R': {corr_B_R:.4f}")
...
# 输入C', N', B', R'的归一化后的值
C_prime = C_normalized
N_prime = N_normalized
B_prime = B_normalized
R_prime = R_normalized

# 创建自变量矩阵X，并添加截距项
X = np.column_stack((C_prime, N_prime, B_prime))
X = sm.add_constant(X)  # 加入截距项W_0

# 创建线性回归模型
model = sm.OLS(R_prime, X)
results = model.fit()
print('results:',results)

# 获取回归系数
intercept = results.params[0]  # W_0
W_C = results.params[1]
W_N = results.params[2]
W_B = results.params[3]
...
print(f"Coefficient for C' (W_C): {W_C}")
print(f"Coefficient for N' (W_N): {W_N}")
print(f"Coefficient for B' (W_B): {W_B}")
print(f"Residuals (e): {residuals}")


model = sm.OLS(R_prime, X).fit()
print(model.summary())
