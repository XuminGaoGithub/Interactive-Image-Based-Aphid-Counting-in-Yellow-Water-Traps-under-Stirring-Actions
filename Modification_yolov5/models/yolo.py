...
from models.inceptionnext import *
...
### DecoupledHead (from yolov6)
# refer to:https://www.bilibili.com/video/BV1jv4y1774u/?spm_id_from=333.999.0.0&vd_source=400fac036b19d9beb706a8e110d9ebf0
...
### Specific Context Decoupling
# refer to https://www.bilibili.com/video/BV1nT411z7on/?spm_id_from=333.999.0.0&vd_source=400fac036b19d9beb706a8e110d9ebf0
...
class Model(nn.Module):
    ...
    def __init__(self, cfg='yolov5s.yaml', ch=3, nc=None, anchors=None):  # model, input channels, number of classes
        ...
        # if isinstance(m, Detect): #Original_Detect
        if isinstance(m, (Detect, Decoupled_Detect, TSCODE_Detect)):  # Decoupled_Detect
            ## DCNV3-CUDA
            #print("cfg",cfg)
            #print(type(cfg))
            #print('-------------------------------------------------------------')
            if cfg == 'models/yolov5s-DCN-V3-2.yaml':
                s = 256  # 2x min stride
            ...
            elif cfg == 'models/yolov5_EfficientFormerV2.yaml' or \
                    cfg == 'models/yolov5_swin_transformer_tiny.yaml' or \
                    cfg == 'models/yolov5_EfficientFormerV2-l.yaml':
                    #cfg == 'models/yolov5_efficientViT.yaml' or \
                    #cfg == 'models/yolov5_efficientViT-b3.yaml':
                s = 640  # EfficientFormerV2,transform embedding issues that sensitive to the input size
                m.inplace = self.inplace
    ...
    def _profile_one_layer(self, m, x, dt):
        # c = isinstance(m, Detect)  # is final layer, copy input as inplace fix #Original_Detect
        c = isinstance(m, (Detect, Decoupled_Detect, TSCODE_Detect))  # Decoupled_Detect
    ...
    ### For Specific Context Decoupling + Decoupled_head #Decoupled_Detect
    '''
    def _initialize_biases(self, cf=None):  # initialize biases into Detect(), cf is class frequency
        # https://arxiv.org/abs/1708.02002 section 3.3
        # cf = torch.bincount(torch.tensor(np.concatenate(dataset.labels, 0)[:, 0]).long(), minlength=nc) + 1.
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in {
                ...
                BottleneckCSP, C3, C3TR, C3SPP, C3Ghost, nn.ConvTranspose2d, DWConvTranspose2d, C3x, CBAM,CoordAtt,SimAM,ShuffleAttention, CrissCrossAttention,
            C3_Res2Block, CSPStage, C3RFEM,CoT3,CTR3, ODConv_3rd}:
            c1, c2 = ch[f], args[0]
            if c2 != no:  # if not output
                c2 = make_divisible(c2 * gw, 8)

            args = [c1, c2, *args[1:]]
            if m in {BottleneckCSP, C3, C3TR, C3Ghost, C3x, SimAM, C3_Res2Block,ShuffleAttention,CrissCrossAttention,CSPStage,
                     C3RFEM, CoT3}:
        ...
        # TODO: channel, gw, gd
        # elif m in {Detect}: #Original_head
        elif m in {Detect, Decoupled_Detect, TSCODE_Detect}:  # Decoupled_Detect
        ...
        # EfficientFormerV2_backbone
        # refer to https://www.bilibili.com/video/BV1da4y1g7KT/?vd_source=400fac036b19d9beb706a8e110d9ebf0
        #note that if you change the input size, need to modify related size from EfficientForm.py, yolo.py, torch_utils.py
        elif m in {efficientformerv2_s0,efficientformerv2_s1,efficientformerv2_s2,efficientformerv2_l}:
            m = m(*args)
            c2 = m.channel

        # efficientvit_backbone
        # refer to https://www.bilibili.com/video/BV1xk4y1L7Gu/?spm_id_from=333.337.search-card.all.click&vd_source=400fac036b19d9beb706a8e110d9ebf0
        elif m in {efficientvit_b0, efficientvit_b1, efficientvit_b2, efficientvit_b3}:
            m = m(*args)
            c2 = m.channel

        # fasternet_backbone
        # refer to https://www.bilibili.com/video/BV1ra4y1K77u/?vd_source=400fac036b19d9beb706a8e110d9ebf0
        elif m in {fasternet_s, fasternet_m, fasternet_l, fasternet_t0, fasternet_t0, fasternet_t1, fasternet_t2}:
            m = m(*args)
            c2 = m.channel

        # fasternet_backbone
        # refer to https://www.bilibili.com/video/BV1ra4y1K77u/?vd_source=400fac036b19d9beb706a8e110d9ebf0
        elif m in {inceptionnext_tiny, inceptionnext_small, inceptionnext_base, inceptionnext_base_384}:
            m = m(*args)
            c2 = m.channel

        # SwinTransformer_Tiny
        # refer to https://www.bilibili.com/video/BV1WX4y1a7ea/?vd_source=400fac036b19d9beb706a8e110d9ebf0
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in (DCNV3_YoLo, C3_DCNV3, ODConv_3rd,C3STR,ODConv2d,DRConv2d,CoT3, SwinV2_CSPB, CTR3, Conv, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, MixConv2d, Focus, CrossConv,
                 BottleneckCSP, C3, C3TR, C3SPP, C3Ghost, nn.ConvTranspose2d, DWConvTranspose2d, C3x,DCNConv):
            c1, c2 = ch[f], args[0]
            if c2 != no:  # if not output
                c2 = make_divisible(c2 * gw, 8)

            args = [c1, c2, *args[1:]]
            if m in [C3_DCNV3, C3STR, CoT3, SwinV2_CSPB, BottleneckCSP, C3, C3TR, C3Ghost, C3x]:
                args.insert(2, n)  # number of repeats
                n = 1
        elif m is nn.BatchNorm2d:
            args = [ch[f]]
...
if __name__ == '__main__':
    ...
    # Create model
    im = torch.rand(opt.batch_size, 3, 640, 640).to(device) ### what meaning?
