    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for n in range(10,12):
            ...
            if mulpicplus == 1:
                ...
                xsz = img.shape[2]
                ysz = img.shape[3]
                #print('img.shape[2],img.shape[3]',img.shape[2],img.shape[3])
            ...
            for i, det in enumerate(pred):
                ...
                im1 = np.zeros((img.shape[2], img.shape[3], 3), dtype=np.uint8)
        ...
        if len(boxes):
            for i in range (len(boxes)):
                ...
                if scores[i] > 0.5: #screen threshold after WFB
                    out.append([float(boxes[i][0] * im1.shape[1]), float(boxes[i][1] * im1.shape[0]),
                                   float(boxes[i][2] * im1.shape[1]), float(boxes[i][3] * im1.shape[0]),
                                   float(scores[i]), float(labels[i])])
