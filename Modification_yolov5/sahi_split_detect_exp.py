...
def run(
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        if mulpicplus == "1":
            ...
            #src = cv2.imread('/home/xumin/yolov5/runs/slicing/640640.png')
            #cropped_image = src[384:640, 384:640]
            #cv2.imwrite('8.png', cropped_image)

            #save the resized image (640*640) from the original images
            ##save the sliced block pics
            #src = cv2.imread(img_pah)
            src = cv2.imread(path)
            img_copy = src.copy()
            img_copy = cv2.resize(img_copy, (width, height))
            src = cv2.resize(src, (width, height))
            cv2.imwrite('/home/xumin/yolov5/runs/slicing/' + '640640' + '.png', src)
            ...
            for i in range(len(slice_bboxes)):
                ...
                cropped_image = src[slice_bboxes[i][0]:slice_bboxes[i][2], slice_bboxes[i][1]:slice_bboxes[i][3]]
            ...
            #for i in range(len(slice_bboxes)):
                ...
                for j, det in enumerate(pred_slice):
                    ...
                    cv2.imwrite('/home/xumin/yolov5/runs/slicing/' + 'pred' +str(i) + '.png', cropped_image)
        ...
        for i, det in enumerate(pred):  # per image
            ...
            cv2.putText(im0, count_number, (150, 150), cv2.FONT_HERSHEY_SIMPLEX, 5, (255, 0,0), 5)
