...
def run(
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        if mulpicplus == "1":
            ...
            cv2.imwrite('/home/xumin/yolov5/runs/slicing/' + '640480' + '.jpg', src)
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if 200 > len(det): # accpet the detection result of yolov5
            #if False:
                ...
                checkpoint = torch.load('/home/xumin/phd/CSRNet-pytorch-master/result/aphids_images_63_second_reverse_augument-trainingset/0model_best.pth.tar')
                ...
                img = cv2.imread(img_path, 1)
                img = cv2.resize(img, (2000, 2000))
                density_map = cv2.imread(save_path, 0)

                mask = np.zeros_like(img)
                img_row = img.shape[0]
                img_col = img.shape[1]

                # iterate the pic and modify the value of piexls
                for i in range(img_row):
                    for j in range(img_col):

                        # if density_map[i,j,0]==128 and density_map[i,j,1]==65 and density_map[i,j,2]==115:
                        # mask[i,j,0]=img[i,j,0]
                        # mask[i,j,1] = img[i,j,1]
                        # mask[i,j,2] = img[i,j,2]

                        if density_map[i, j] <= 10:
                            mask[i, j, 0] = img[i, j, 0]
                            mask[i, j, 1] = img[i, j, 1]
                            mask[i, j, 2] = img[i, j, 2]

                        else:
                            # print('density_map[i,j]:',density_map[i,j])
                            # mask[i, j, 0] = 0.7*img[i, j, 0] + 0.3 * (128)
                            # mask[i, j, 1] = 0.7*img[i, j, 1] + 0.3 * (65)
                            # mask[i, j, 2] = 0.7*img[i, j, 2] + 0.3 * (115)

                            # mask[i, j, 0] = 0.8*img[i, j, 0] + ((density_map[i, j]/255.0)*128)
                            # mask[i, j, 1] = 0.8*img[i, j, 1] + ((density_map[i, j]/255.0)*65)
                            # mask[i, j, 2] = 0.8*img[i, j, 2] + ((density_map[i, j]/255.0)*115)

                            mask[i, j, 0] = 0.8 * img[i, j, 0] + 0.2 * ((density_map[i, j] / 255.0) * 0)
                            mask[i, j, 1] = 0.8 * img[i, j, 1] + 0.2 * ((density_map[i, j] / 255.0) * 0)
                            mask[i, j, 2] = 0.8 * img[i, j, 2] + 0.2 * ((density_map[i, j] / 255.0) * 255)
                            ...
                            if mask[i, j, 2] > 255:
                                mask[i, j, 2] = 255
                            # print('mask[i,j]:', mask[i, j, 0], mask[i, j, 1], mask[i, j, 2])
                        
                count_number = "Aphids:{}".format(round(output))
                cv2.putText(mask, count_number, (150, 150), cv2.FONT_HERSHEY_SIMPLEX, 3, (255, 0, 0), 7)
