...
from utils.metrics import val_bbox_iou
        ...
        for c in cls.unique():
            ...
            if soft_nms:
                ...
                while len(dc):
                    ...
                    if len(dc) == 1:
                        break
                    iou = val_bbox_iou(dc[0], dc[1:]) # Soft NMS
                    #iou = val_bbox_iou(dc[0], dc[1:], DIoU=True) #Soft DIoU-NMS
...
def apply_classifier_aphid(x, model, img, im0):
    # Apply a second stage classifier to YOLO outputs
    # Example model = torchvision.models.__dict__['efficientnet_b0'](pretrained=True).to(device).eval()
    im0 = [im0] if isinstance(im0, np.ndarray) else im0
    for i, d in enumerate(x):  # per image
        if d is not None and len(d):
            ...
            b[:, 2:] = b[:, 2:] * 1.3 + 30  # pad
            d[:, :4] = xywh2xyxy(b).long()
            #print('Before_covert_boxes:', d[:, :4])

            # Rescale boxes from img_size to im0 size
            #scale_coords(img.shape[2:], d[:, :4], im0[i].shape)
            coords = scale_coords(img.shape[2:], d[:, :4], im0[i].shape)
            #print('After_covert_boxes:', coords)

            # Classes
            pred_cls1 = d[:, 5].long()
            print('pred_cls1:', pred_cls1)
            ims = []
            for a in d:
                #cutout = im0[i][int(a[1]):int(a[3]), int(a[0]):int(a[2])]
                ...
                cutout = im0[i][y1:y2, x1:x2]
                if cutout is None or cutout.size == 0:
                    print(f"Warning: cutout is empty, skipping resize")
                else:
                    # 打印 cutout 的形状
                    print(f"cutout shape: {cutout.shape}")

                # 检查 cutout 是否为空或过小
                if cutout is None or cutout.size == 0 or cutout.shape[0] < 10 or cutout.shape[1] < 10:
                #if cutout is None:
                    print(f"Warning: cutout is empty or too small, skipping resize. Shape: {cutout.shape}")
                else:
                    im = cv2.resize(cutout, (224, 224))  # BGR

                    #im = cv2.resize(cutout, (224, 224))  # BGR
                    if im is None or im.size == 0:
                        print(f"Warning: im is empty after resize, skipping BGR to RGB conversion")
                    else:
                        im = im[:, :, ::-1]  # BGR to RGB
            ...
            ims = torch.stack(ims)
            #print('img:', img.size())
            # Ensure ims is on the correct device
            ims = ims.to(d.device)
            ...
            pred_cls2_flipped = 1 - pred_cls2
            print('pred_cls2_flipped:', pred_cls2_flipped)
            x[i] = x[i][pred_cls1 == pred_cls2_flipped]  # retain matching class detections

            #softmax_output = F.softmax(pred_cls2, 1).cpu().data.numpy()
            #softmax_output[0][0], softmax_output[0][1]
            #print('np, pp:', softmax_output[0][0], softmax_output[0][1])


    return x

def apply_classifier_aphid_SAHI(x, model, img, im0):
    # Apply a second stage classifier to YOLO outputs
    # Example model = torchvision.models.__dict__['efficientnet_b0'](pretrained=True).to(device).eval()
    im0 = [im0] if isinstance(im0, np.ndarray) else im0
    for i, d in enumerate(x):  # per image
