...
from utils.general import (LOGGER, check_dataset, check_img_size, check_requirements, check_yaml,
                           ...
                           scale_coords, xywh2xyxy, xyxy2xywh, apply_classifier_aphid)
...
def process_batch(detections, labels, iouv):
    ...
    #print('detections[:, :4]', detections[:, :4])
    correct_class = labels[:, 0:1] == detections[:, 5]
    np.set_printoptions(precision=6)
    #print('iou,correct_class',iou,correct_class)
    #print('iou',iou)
    #print('iouv', iouv)
...
def run(
    ...
    #classify = False
    if classify:
        modelc = torch.load('/home/newdrive/Phd/Modification_yolov5/dataset/voc2057_cls/Thres0.25_APiou0.5_selected_2nd/model/resnet18_best.pth')  # initialize
        model.cuda()
        model.eval()
    ...
    if not training:
        ...
        #256x256 if the size for training is that
        #print('model.warmup_imgsz:',imgsz)
        model.warmup(imgsz=(1 if pt else batch_size, 3, imgsz, imgsz))  # warmup. the size doesn't affect the test results
        #model.warmup(imgsz=(1 if pt else batch_size, 3, 256, 256))  # warmup
    ...
    for batch_i, (im, targets, paths, shapes) in enumerate(pbar):
        ...
        if mulpicplus == "1":
            ...
            # some image_size is < 640,so will be stuck if we use slice_pics 640x640
            if im.shape[2]<640 and im.shape[3]<640:
            ...
            for i in range(len(slice_bboxes)):
                ...
                #print('discrc.shape:',dicsrc.shape)
        ...
        for si, pred in enumerate(out):
            ...
            #print('predn1',predn)
            # no need to do this,as use original image, otherwise it will case error for calculating acc
            ...
            if nl:
                ...
                # coords=scale_coords(im[si].shape[1:], tbox, shape, shapes[si][1])  # native-space labels
                #print('coords:',coords)
    ...
    #print('len(stats),stats[0].any()',len(stats),stats[0].any())


    if len(stats) and stats[0].any():
    #if len(stats):
        tp, fp, p, r, f1, ap, ap_class = ap_per_class(*stats, plot=plots, save_dir=save_dir, names=names)
        #print('tp, fp, p, r, f1, ap, ap_class:',tp, fp, p, r, f1, ap, ap_class)
